...
`main.py` is the entry point. Each mode runs one part of the pipeline:
    ...
    python3 main.py classifier  # train a decision tree to predict the action
...
- `classifier.py` - generates labelled (features, action) data from the baseline
  playing in RLCard and trains a decision tree to predict the action.
...
- Simple supervised classifier (decision tree) that predicts the action from
  phi(state), trained on data generated by the baseline. It reaches about 99%
  test accuracy, i.e. a fast approximation of the baseline policy. It imitates
  our baseline heuristic, not real professional play.
